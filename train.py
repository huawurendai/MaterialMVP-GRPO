...
from materialmvp.lora_utils import save_lora_checkpoint, setup_lora_for_model
import warnings
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "--validate_only",
        action="store_true",
        help="only run validation from a checkpoint",
    )
    parser.add_argument(
        "-b",
        "--base",
...
class LoRACheckpointCallback(Callback):
    def __init__(self, ckptdir, save_every_n_steps=100):
        self.ckptdir = ckptdir
        self.save_every_n_steps = save_every_n_steps

    @rank_zero_only
    def _save_lora(self, trainer, pl_module, filename):
        if getattr(pl_module, "lora_report", None) is None:
            return
        os.makedirs(self.ckptdir, exist_ok=True)
        path = os.path.join(self.ckptdir, filename)
        save_lora_checkpoint(path, pl_module.unet, pl_module.lora_config_resolved)
        rank_zero_print(f"Saved LoRA checkpoint: {path}")

    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if self.save_every_n_steps > 0 and trainer.global_step > 0 and trainer.global_step % self.save_every_n_steps == 0:
            self._save_lora(trainer, pl_module, f"lora_step_{trainer.global_step:08d}.pt")

    def on_train_end(self, trainer, pl_module):
        self._save_lora(trainer, pl_module, "lora_last.pt")
...
class MetricsCSVCallback(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        row = {"step": trainer.global_step, "epoch": trainer.current_epoch}
        for key in self.fieldnames:
            if key in row:
                continue
            value = metrics.get(key)
            if value is None:
                value = metrics.get(f"{key}_step")
            if value is None:
                row[key] = ""
...
class GradUsageCallback(Callback):
    def __init__(self, logdir, every_n_steps=100, filename="grad_report.jsonl"):
        self.path = os.path.join(logdir, filename)
    ...
    def on_after_backward(self, trainer, pl_module):
        ...
        zero_grad = []
        grad_norm = {}
        for name, param in pl_module.unet.named_parameters():
...
if __name__ == "__main__":
    ...
    if getattr(model, "lora_config", None) and model.lora_config.get("enabled", False):
        setup_lora_for_model(model, model.lora_config)
    ...
    default_callbacks_cfg = {
        ...
        "lora_checkpoint": {
            "target": "train.LoRACheckpointCallback",
            "params": {
                "ckptdir": ckptdir,
        ...
        "metrics_csv": {
            "target": "train.MetricsCSVCallback",
            "params": {
                "logdir": logdir,
            },
        },
        "grad_usage": {
            "target": "train.GradUsageCallback",
            "params": {
                "logdir": logdir,
                "every_n_steps": 100,
            },
    ...
    if opt.validate_only:
        if opt.resume is None:
            raise ValueError("--validate_only requires --resume to load the trained checkpoint.")
        os.makedirs(os.path.join(logdir, "images_val"), exist_ok=True)
        trainer.validate(model, dataloaders=data.val_dataloader(), ckpt_path=opt.resume)
        exit(0)
